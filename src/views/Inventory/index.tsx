                                            ...
                                            return (
                                                <Fragment key={key}>
                                                    <InventoryBlook
                                                        ...
                                                        shiny={false}
                                                        big={blook.isBig}
                                                        locked={getBlookAmount(blook.id, false) <= 0}
                                                        amount={getBlookAmount(blook.id, false)}
                                                        onClick={() => {
                                                            if (getBlookAmount(blook.id, false) <= 0) return;

                                                            setSelectedType(SelectedTypeEnum.BLOOK);
                                                            setSelected(user.blooks.find((ub) => ub.blookId === blook.id && !ub.shiny) || null);
                                                        }}
                                                    />

                                                    {getBlookAmount(blook.id, true) > 0 &&
                                                        <InventoryBlook
                                                            blook={blook}
                                                            shiny={true}
                                                            big={blook.isBig}
                                                            locked={false}
                                                            amount={getBlookAmount(blook.id, true)}
                                                            onClick={() => {
                                                                setSelectedType(SelectedTypeEnum.BLOOK);
                                                                setSelected(user.blooks.find((ub) => ub.blookId === blook.id && ub.shiny) || null);
                                                            }}
